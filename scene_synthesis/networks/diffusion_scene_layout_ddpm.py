...
class DiffusionSceneLayout_DDPM(Module):
    ...
    # @torch.no_grad()
    # def arrange_scene(self, room_mask, num_points, point_dim, input_boxes, device="cpu", clip_denoised=False, batch_seeds=None):
        
    #     samples = self.sample(room_mask, num_points, point_dim, batch_size=1, input_boxes=input_boxes,clip_denoised=clip_denoised, batch_seeds=batch_seeds)
        
    #     return self.delete_empty_from_network_samples(samples, device=device,keep_empty=False)
    @torch.no_grad()
